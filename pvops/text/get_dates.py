...
def get_dates(document, om_df, ind, col_dict, print_info, infer_date_surrounding_rows=True):
    ...
    try:
        ...
        if isinstance(row[EVENTSTART_COLUMN], float) and np.isnan(row[EVENTSTART_COLUMN]):
            ...
            if infer_date_surrounding_rows:
                ...
                while find_valid is False and no_base_date_found is False:
                    ...
                    if ind_behind >= 0:
                        ...
                        if isinstance(row_behind[EVENTSTART_COLUMN], float) and np.isnan(row_behind[EVENTSTART_COLUMN]):
                            pass
                        else:
                            basedate = list(datefinder.find_dates(row_behind[EVENTSTART_COLUMN]))[0]
                            find_valid = True
                            continue
                    ...
                    if ind_ahead < om_df_len:
                        ...
                        if isinstance(row_ahead[EVENTSTART_COLUMN], float) and np.isnan(row_ahead[EVENTSTART_COLUMN]):
                            pass
                        else:
                            basedate = list(datefinder.find_dates(row_ahead[EVENTSTART_COLUMN]))[0]
                            find_valid = True
    ...
    for mtch in matches:
        try:
            if (mtch > datetime.strptime('01/01/1970', '%m/%d/%Y')) and (mtch < datetime.now() + timedelta(days=365*100)):
