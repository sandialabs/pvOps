...
def visualize_cluster_entropy(doc2vec, eval_kmeans, om_df, data_cols, ks, cmap_name='brg'):
    ...
    for i, col in enumerate(cols):
        ...
        doc2vec_data = [TaggedDocument(words=x, tags=[str(i)]) for i, x in enumerate(tokenized_data)]
        ...
        plt.plot(ks, sse, color=cmap(2*i), marker ='o', label=f'Doc2Vec + {col} entropy')
        ...
        for true_k in ks:
            ...
            sse.append(km.inertia_)
            clusters.append(km.labels_)
        plt.plot(ks, sse, color=cmap(2*i+1), marker ='o', label=f'TF-IDF + {col} entropy')
