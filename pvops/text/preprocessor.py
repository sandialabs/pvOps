...
def preprocessor(om_df, lst_stopwords, col_dict, print_info = False, extract_dates_only = False):
    ...
    if print_info:
        print(f'len clean corpus: {len(clean_corpus)}, len deduced dates: {len(dates_extracted)}')
        print(f"num_total {n_total}, num_nans {n_nans}, num_fails_date {n_fails_date}, num_fails_prep {n_fails_prep}, tally {tally}")
    ...
    for ind,row in df.iterrows():
        ...
        try:
            ...
            for dt in nlp_dates:
                ...
                if abs((date-dt).total_seconds()) < 3.154e+7:
                    fltrd.append(dt)
