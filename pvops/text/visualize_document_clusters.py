...
def visualize_document_clusters(cluster_tokens, min_frequency = 20):
    ...
    for wd in unique_words:
        ...
        clusters_this_wd = [idx for idx,words_in_cluster in enumerate(all_words_of_interest) if wd in words_in_cluster]
    ...
    ax = df['freq'].plot(kind='barh', figsize=(20,14),
                        color="coral", fontsize=13)

    xbias = 0.3
    ybias = 0.0
    for idx,i in enumerate(ax.patches):
        ax.text(i.get_width()+xbias, i.get_y()+ybias, \
                filter_word_list[idx], fontsize=15,
                color='dimgrey')
