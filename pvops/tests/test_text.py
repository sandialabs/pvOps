...
def test_get_dates():
    ...
    assert answer == get_dates(df['Document'].iloc[0], df, 0, {"data":"Document", "eventstart":"Date"}, False)

    answer = [datetime.datetime.strptime("2021/03/05 00:00:00", "%Y/%m/%d %H:%M:%S"),
              datetime.datetime.strptime("2022/04/07 00:00:00", "%Y/%m/%d %H:%M:%S")]
    assert answer == get_dates(df['Document'].iloc[1], df, 1, {"data":"Document", "eventstart":"Date"}, False)

def test_visualize_attribute_timeseries():

    dates = pd.Series(["2020/01/23 12:34:56", 
    "2020/01/24 12:34:56",
    "2020/01/25 12:34:56",
    ])
    ...
    df = pd.DataFrame({
        "labels": ["A word", "B word", "C word"],
        "date": dates
    })

    fig = visualize_attribute_timeseries(df, {"label":"labels", "date":"date"}, date_structure='%Y-%m-%d')
...
def test_visualize_attribute_connectivity():
    ...
    fig, edges = visualize_attribute_connectivity(df, om_col_dict,
                figsize=(10,8),
                edge_width_scalar=2,
