...
class IVProcessor():
    ...
    def train_met_fit(self, simulation_function):
        ...
        self.isc_meas_lm = linear_model.LinearRegression().fit(
            meas_Iscs.reshape(-1, 1), self.Irrs.reshape(-1, 1))
        self.isc_sim_lm = linear_model.LinearRegression().fit(
            self.sim_Iscs.reshape(-1, 1), self.Irrs.reshape(-1, 1))
        self.voc_meas_lm = linear_model.LinearRegression().fit(
            meas_Vocs.reshape(-1, 1), Tcs.reshape(-1, 1))
        self.voc_sim_lm = linear_model.LinearRegression().fit(
            self.sim_Vocs.reshape(-1, 1), Tcs.reshape(-1, 1))
    ...
    def met_fit(self, Vsim, Isim, Irr, Tc, meas_Isc, meas_Voc):
        ...
        E_sim_pred = self.isc_sim_lm.predict(
            np.asarray([meas_Isc]).reshape(-1, 1))[0][0]
        ...
        tc_sim_pred = self.voc_sim_lm.predict(
            np.asarray([meas_Voc]).reshape(-1, 1))[0][0]
    ...
    def f(self, params):
        ...
        sim.add_preset_conditions(
            'complete', condition, save_name=f'mod_case_{self.counter}')
        sim.build_strings({f'str_case_{self.counter}': [
                          f'mod_case_{self.counter}']*12})
    ...
    # , meas_I, meas_V, g, tc):
    def fit_params(self, cell_parameters, module_parameters, n_mods, user_func=None):
