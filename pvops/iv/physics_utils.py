def calculate_params(v,c):
    ...
    if type(isc_lim) == float:
        ...
        isc_size = isc_lim
    isc_lm = linear_model.LinearRegression().fit(v[:isc_size].reshape(-1, 1), c[:isc_size].reshape(-1, 1))
...
def gt_correction(v,i,gact,tact,cecparams,n_units=1,option=1):
    ...
    if option == 1:
        ...
        vref = v - rs * (iref - i) - k1 * iref * (tref - tact) + beta * (tref - tact)

    elif option == 2:
        # IEC60891 Procedure 2
        iref = i * (1 + alpha * (tref-tact)) * (gref / gact)
        vref = v + voc * (beta * (tref-tact) + alpha * math.log(gref/gact)) - rs * (iref-i) - k2 * iref * (tref-tact)
