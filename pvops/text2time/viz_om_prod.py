...
def viz_om_prod(prod_df, om_df, prod_col_dict, om_col_dict, prod_fldr, Ecumu, BEcumu, samp_freq='H', pshift=0.0, baselineflag=True):
    '''
    ...
    prod_baseline = prod_col_dict['baseline'] #if none is provided, using iec_calc() is recommended
    ...
    for i in prod_df.index.get_level_values(0).unique():
        ...
        if Ecumu:
            ...
            enrg_site = prod_df.loc[i, prod_ener].resample(samp_freq, label='left').sum()
        ...
        if BEcumu: #baseline energy cumulative over time
            baseline_site = prod_df.loc[i, prod_baseline].resample(samp_freq, label='left').max() -\
            prod_df.loc[i, prod_baseline].resample(samp_freq, label='left').min()
        else: #baseline energy is on a per time-step basis
            baseline_site = prod_df.loc[i, prod_baseline].resample(samp_freq, label='left').sum()
        ...
        # indices to dates
        E_dips_nearOM_Indices = [np.argmin(abs(edips_all_dates-xx)) for xx in om_df.loc[i].index]
        ...
        fig = go.Figure(layout_yaxis_range=[-5, enrg_site.max()*om_reg_h*om_reg_hcorr])
        ...
        if baselineflag:
            fig.add_trace(go.Scatter(x=baseline_site.index, y=baseline_site.values, name=baseline_name))
        ...
        for j in range(len(om_df.loc[i])):
            fig.add_trace(dict(
                    ...
                    y=om_df.loc[i, 'perfval_plotcol'].values[0:2]*om_reg_h*om_reg_hcorr,
                    mode="markers+lines",
                    line=dict(width=0),
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="markers",
            hovertemplate='Start: ' + '%{x} <br>' +
                'WO#: ' + om_df.loc[i, om_wo_id].astype(str) + '<br>' +
                'Type: ' + om_df.loc[i, om_wtype].astype(str) + '<br>' +
                'Asset: ' + om_df.loc[i, om_asset].fillna('Asset_NA').astype(str) + '<br>' +
                'Nearest Prod Dip:  ' + om_df.loc[i, 'corr_perfDip'].dt.strftime('%b %d, %Y'),
            name='OM_start'
            ))
        ...
        fig.add_trace(go.Scatter(
            ...
            mode="markers",
            hovertemplate='End: ' + '%{x} <br>' +
                'WO#: ' + om_df.loc[i, om_wo_id].astype(str) + '<br>' +
                'Type: ' + om_df.loc[i, om_wtype].astype(str) + '<br>' +
                'Asset: ' + om_df.loc[i, om_asset].fillna('Asset_NA').astype(str),
            name='OM_end'
            ))
