...
def iec_calc(prod_df, prod_col_dict, meta_df, meta_col_dict, gi_ref=1000.):
    '''
    ...
    prod_df[prod_dcsize] = prod_df.loc[:, prod_site].apply(lambda x: meta_df.loc[x, meta_size])
    ...
    for sid in prod_df.loc[:,prod_site].unique():
        ...
        tstep = tstep/np.timedelta64(1, 'h')  # Converting the time-step to float (representing hours) to
        # arrive at kWh for the iecE calculation

        prod_df.loc[mask, prod_iec] = prod_df.loc[mask,prod_dcsize] * prod_df.loc[mask, prod_irr] * tstep/gi_ref
