...
def om_nadate_process(om_df, om_col_dict, om_dendflag='drop'):
    ...
    om_df.dropna(subset=[om_date_s], inplace=True) #drops rows with om_date_e of NA in om_df
    ...
    if om_dendflag == 'drop':
        om_df.dropna(subset=[om_date_e], inplace=True) #drops rows with om_date_e of NA in om_df
    elif om_dendflag == 'today':
        om_df[om_date_e].fillna(pd.to_datetime(str(datetime.now())[:20]), inplace=True) #replacing NANs with today's date
    else:
        None
