...
def overlapping_data(prod_df, om_df, prod_col_dict, om_col_dict):
    ...
    for rid in prod_df.index.unique():
        if rid in om_df.index.unique():
            ...
            omtail_gt_phead_mask = om_df.loc[rid, om_date_e] >= min(prod_df.loc[rid][prod_ts])
            ...
            perf_gt_omhead_mask = prod_df.loc[rid][prod_ts].dt.ceil('D') >= min(om_datestart_check)
            perf_lt_omtail_mask = prod_df.loc[rid][prod_ts].dt.floor('D') <= max(om_dateend_check)

            # Creating NEW DataFrames using masks generated above and concatenate to "_commondates" DFs
            if isinstance(pd.to_datetime(om_df.loc[rid][om_date_s]), pd.Series):
                om_overlap_section = om_df.loc[rid][(omtail_gt_phead_mask) & (omhead_lt_ptail_mask)]
                om_df_commondates = pd.concat([om_df_commondates, om_overlap_section])
            else:
                om_overlap_section = om_df.loc[[rid]][[(omtail_gt_phead_mask) & (omhead_lt_ptail_mask)]]
                om_df_commondates = pd.concat([om_df_commondates, om_overlap_section])

            perf_overlap_section = prod_df.loc[rid][(perf_gt_omhead_mask) & (perf_lt_omtail_mask)]
