...
# Visualizations
from pvops.text import visualize_word_frequency_plot, visualize_attribute_connectivity, visualize_attribute_timeseries, visualize_cluster_entropy, visualize_document_clusters
# Preprocessing
...
class Example:
    ...
    def visualize_attribute_timeseries(self, DATE_COLUMN):
        ...
        fig = visualize_attribute_timeseries.visualize_attribute_timeseries(df, col_dict)
        return fig
    ...
    def visualize_cluster_entropy(self, cols):
        ...
        fig = visualize_cluster_entropy.visualize_cluster_entropy(
                                doc2vec_model,
                                eval_kmeans,
                                self.df, cols, ks, cmap_name='brg')
        return fig

    def extract_dates(self, DATA_COLUMN, EVENTSTART_COLUMN, SAVE_DATA_COLUMN='CleanDesc', SAVE_DATE_COLUMN='ExtractedDates'):
        ...
        try:
            self.df = preprocessor.preprocessor(self.df, None, col_dict, print_info = False, extract_dates_only = True)
    ...
    def prep_data_for_ML(self, DATA_COLUMN, EVENTSTART_COLUMN, SAVE_DATA_COLUMN='CleanDesc', SAVE_DATE_COLUMN='ExtractedDates'):
        ...
        lst_stopwords = create_stopwords.create_stopwords(["english"],
        ...
        try:
            self.df = preprocessor.preprocessor(self.df, lst_stopwords, col_dict, print_info = False)
        except Exception as e:
            print(e)
            print(traceback.format_exc())
    ...
    def visualize_freqPlot(self, LBL_CAT=None, DATA_COLUMN='CleanDesc',**graph_aargs):
        ...
        if LBL_CAT == None:
            ...
            words = ' '.join(self.df[self.df[self.LABEL_COLUMN] == LBL_CAT][DATA_COLUMN].tolist())
            num_rows = len(self.df[self.df[self.LABEL_COLUMN] == LBL_CAT].index)
        else:
            raise Exception(f"An invalid label category (LBL_CAT) was passed. Please pass a value in the following set:\n{set(self.df[self.LABEL_COLUMN].tolist())}")

        tokenized = nltk.word_tokenize(words)
        fig = visualize_word_frequency_plot.visualize_word_frequency_plot(tokenized, title=str(LBL_CAT) + f' (Num. Docs: {num_rows})',**graph_aargs)
        return fig
    ...
    def visualize_document_clusters(self, min_frequency=20, DATA_COLUMN='CleanDesc'):
        ...
        cluster_words = [' '.join(self.df[self.df[self.LABEL_COLUMN] == LBL_CAT][DATA_COLUMN].tolist()) for LBL_CAT in all_labels]
        num_rows = len(self.df[DATA_COLUMN].index)
        cluster_tokens = [nltk.word_tokenize(words) for words in cluster_words]
        fig = visualize_document_clusters.visualize_document_clusters(cluster_tokens, min_frequency=min_frequency)
        return fig
    ...
    def visualize_attribute_connectivity(self, om_col_dict, **networkxDrawAargs):
        ...
        fig, edges = visualize_attribute_connectivity.visualize_attribute_connectivity(df_filtered, om_col_dict, **networkxDrawAargs)
        return fig, edges
    ...
    def _classify(self,
        ...
        try:
            ...
            # sort values
            results_df = results_df.sort_values(['mean_score'], ascending= not self.greater_is_better)
        except Exception as e:
            print(e)
            print(traceback.format_exc())
    ...
    def classify_supervised(self, embedding='tfidf', n_cv_splits = 5, subset_example_classifiers = None, setting = 'normal', user_defined_classes = None, user_defined_search_space = None):
        ...
        if embedding == 'tfidf':
            ...
            pipeline_steps = [('doc2vec', Doc2VecModel.Doc2VecModel()),
                              ('clf', None)]
        ...
        if user_defined_classes == None or user_defined_search_space == None:
            ...
            search_space, classes = supervised_classifier_defs.supervised_classifier_defs(setting)
    ...
    def classify_unsupervised(self, embedding='tfidf', n_cv_splits = 5, subset_example_classifiers = None, setting = 'normal', user_defined_classes = None, user_defined_search_space = None):
        ...
        if user_defined_classes == None or user_defined_search_space == None:
            ...
            search_space, classes = unsupervised_classifier_defs.unsupervised_classifier_defs(setting, n_clusters)
    ...
    def predict_best_model(self, ml_type = 'supervised', eval_func = None, PREDICTION_OUTPUT_COL = None, *eval_aargs):
